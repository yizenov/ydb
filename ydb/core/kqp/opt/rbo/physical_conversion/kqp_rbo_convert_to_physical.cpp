...
TExprNode::TPtr ConvertToPhysical(TOpRoot& root, TRBOContext& rboCtx) {
    ...
    for (const auto& iter : root) {
        ...
        if (op->Kind == EOperator::EmptySource) {
            ...
            // The looked-up rows arrive from the TKqpCnStreamLookup connection with the main table's
            // plain column names; normalize to a stream and rename them to the read's qualified output
            // IUs so downstream operators see the names they expect.
