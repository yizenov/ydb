...
TStatus ComputeTypes(TIntrusivePtr<TOpTableLookup> lookup, TRBOContext& ctx) {
    // The lookup output columns are fetched from the main table, so its row type is that table's
    // row type for FetchColumns, renamed to the lookup's exposed output IUs (same mapping as a read).
